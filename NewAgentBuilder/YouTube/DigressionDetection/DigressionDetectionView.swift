...
@MainActor
class DigressionDetectionViewModel: ObservableObject {
    ...
    var crossRunRegions: [CrossRunDigressionRegion] {
        CrossRunDigressionRegion.buildRegions(from: runs)
    }

    var unanimousRegions: [CrossRunDigressionRegion] {
        crossRunRegions.filter { $0.isUnanimous }
    }

    var divergentRegions: [CrossRunDigressionRegion] {
        crossRunRegions.filter { $0.isDivergent }
    }
    ...
    func runFidelityTest() async {
        ...
        let results = await withTaskGroup(
            ...
            for await runResult in group {
                ...
                await MainActor.run {
                    self.currentRun = collected.count
                    self.currentPhase = "Completed \(collected.count) of \(totalRuns) runs"
                }
            }
            return collected
        }

        runs = results.sorted { $0.runNumber < $1.runNumber }
    ...
    var summaryText: String {
        ...
        var lines: [String] = []
        lines.append("════════════════════════════════════════════════════════════════")
        lines.append("DIGRESSION DETECTION REPORT")
        lines.append("════════════════════════════════════════════════════════════════")
        ...
        let regions = crossRunRegions
        lines.append("WHAT — \(regions.count) digression regions across \(runs.count) runs:")
        lines.append("  Unanimous: \(unanimousRegions.count)")
        lines.append("  Divergent: \(divergentRegions.count)")
        lines.append("")
        ...
        // Region table
        lines.append("WHAT — Region Comparison:")
        lines.append("  \("Type".padding(toLength: 16, withPad: " ", startingAt: 0))\("Range".padding(toLength: 10, withPad: " ", startingAt: 0))\("Content".padding(toLength: 30, withPad: " ", startingAt: 0))R1  R2  R3  Match")
        for region in regions {
            let brief = String((region.briefContent ?? "").prefix(28))
            var runCols = ""
            for i in 1...runs.count {
                runCols += region.perRunAnnotation[i] != nil ? " Y  " : " -  "
            }
            let match = "\(Int(region.consistency * 100))%"
            lines.append("  \(region.primaryType.displayName.padding(toLength: 16, withPad: " ", startingAt: 0))\(region.rangeLabel.padding(toLength: 10, withPad: " ", startingAt: 0))\(brief.padding(toLength: 30, withPad: " ", startingAt: 0))\(runCols)\(match)")
        }
...
struct DigressionDetectionView: View {
    @StateObject private var viewModel: DigressionDetectionViewModel

    init(video: YouTubeVideo) {
        _viewModel = StateObject(wrappedValue: DigressionDetectionViewModel(video: video))
    }
    ...
    var body: some View {
        ...
        .navigationTitle("Digression Detection")
        .task {
    ...
    private var progressSection: some View {
        VStack(spacing: 16) {
            ...
            Text("\(viewModel.currentRun) of \(viewModel.runCount) complete")
                .font(.headline)
    ...
    private var emptyStateView: some View {
        VStack(spacing: 16) {
            if viewModel.sentences.isEmpty {
                ...
                Text("Digression Detection")
                    .font(.headline)
                    .foregroundColor(.secondary)

                Text("Finds digressions in the transcript — sponsor reads, personal asides, tangents, and more. Runs multiple passes and compares results for consistency.")
                    .font(.caption)
                    .foregroundColor(.secondary)
    ...
    private var summaryView: some View {
        VStack(alignment: .leading, spacing: 0) {
            // Stats header
            HStack {
                Text("\(viewModel.crossRunRegions.count) regions across \(viewModel.runs.count) runs")
                    .font(.headline)

                Spacer()

                HStack(spacing: 8) {
                    statBadge("\(viewModel.unanimousRegions.count) unanimous", color: .blue)
                    statBadge("\(viewModel.divergentRegions.count) divergent", color: viewModel.divergentRegions.isEmpty ? .green : .orange)
                }
            ...
            ScrollView {
                LazyVStack(alignment: .leading, spacing: 0) {
                    ...
                    ForEach(viewModel.crossRunRegions) { region in
                        regionRow(region)
                    }
                }
            }
    ...
    private func regionRow(_ region: CrossRunDigressionRegion) -> some View {
        HStack(spacing: 0) {
            // Type with color dot
            HStack(spacing: 4) {
                Circle()
                    .fill(region.primaryType.color)
                    .frame(width: 8, height: 8)
                Text(region.primaryType.displayName)
                    .font(.caption)
                    .lineLimit(1)
            }
            .frame(width: 120, alignment: .leading)
            ...
            // Range
            Text(region.rangeLabel)
                .font(.caption.monospaced())
                .frame(width: 70, alignment: .leading)

            // Brief content
            Text(String((region.briefContent ?? "").prefix(30)))
                .font(.caption)
                .foregroundColor(.secondary)
                .frame(minWidth: 140, alignment: .leading)
                .lineLimit(1)
            ...
            // Per-run checkmarks
            ForEach(1...max(1, viewModel.runs.count), id: \.self) { runNum in
                let detected = region.perRunAnnotation[runNum] != nil
                Image(systemName: detected ? "checkmark.circle.fill" : "minus.circle")
                    .font(.caption)
                    .foregroundColor(detected ? region.primaryType.color : .gray.opacity(0.3))
                    .frame(width: 35)
            }

            // Match %
            Text(region.isUnanimous ? "100%" : "\(Int(region.consistency * 100))%")
                .font(.caption.monospaced())
                .foregroundColor(region.isUnanimous ? .green : .orange)
                .frame(width: 60)
        }
        .padding(.horizontal)
        .padding(.vertical, 6)
