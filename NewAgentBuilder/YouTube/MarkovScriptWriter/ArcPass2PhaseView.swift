//
//  ArcPass2PhaseView.swift
//  NewAgentBuilder
//
//  Phase 4 of the Arc Pipeline: Pass 2 Spine Generation.
//  Re-runs spine generation with original rambling + gap response rambling.
//  Based on ArcSpinePhaseView with Pass 2-specific modifications.
//

import SwiftUI

struct ArcPass2PhaseView: View {
    ...
    @State private var isV6Expanded = false
    @State private var isV7Expanded = false
    @State private var isV8Expanded = false
    @State private var isV9Expanded = false
    @State private var isV10Expanded = false
    @State private var isV11Expanded = false
    @State private var isV12Expanded = false
    @State private var isPreprocessingExpanded = false
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            Button {
                ...
                Label("Back to Respond", systemImage: "arrow.left.circle")
                    .font(.caption)
            }
            ...
            // 4. Shared preprocessing calls
            if let run = vm.currentRun, !run.preprocessingCalls.isEmpty {
                preprocessingCollapsibleSection(run.preprocessingCalls)
            }

            // 5. Per-path results
            ForEach(ArcPath.allCases) { path in
    ...
    private func bindingForPath(_ path: ArcPath) -> Binding<Bool> {
        ...
        case .v6_enrichedSinglePass:      return $isV6Expanded
        case .v7_enrichedContentFirst:    return $isV7Expanded
        case .v8_enrichedFourStep:        return $isV8Expanded
        case .v9_enrichedDynamic:         return $isV9Expanded
        case .v10_enrichedDynamicContent: return $isV10Expanded
        case .v11_freshFourStep:          return $isV11Expanded
        case .v12_freshDynamicContent:    return $isV12Expanded
        }
    }
    ...
    private var configCollapsibleSection: some View {
        CollapsibleSection(
            title: "Pass 2: Enhanced Spine Generation",
            ...
            VStack(alignment: .leading, spacing: 12) {
                Text("Re-run spine generation with your original rambling plus gap response rambling for an improved narrative arc.")
                    .font(.caption)
                    .foregroundStyle(.secondary)

                // Gap rambling context info
                gapRamblingContext

                // First-pass spine status (needed for V6–V10 enrichment)
                firstPassSpineStatus
    ...
    private var gapRamblingContext: some View {
        HStack(spacing: 8) {
            Image(systemName: "text.bubble.fill")
                .foregroundStyle(.green)
            if vm.hasGapRambling {
                Text("Gap rambling included")
                    .font(.subheadline.weight(.semibold))
                Text("\(vm.gapRamblingWordCount) words")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            } else {
                Text("No gap rambling available")
                    .font(.subheadline)
                    .foregroundStyle(.orange)
                Text("Complete the Respond phase first.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
        }
        ...
        .background(vm.hasGapRambling ? Color.green.opacity(0.08) : Color.orange.opacity(0.08))
        .clipShape(RoundedRectangle(cornerRadius: 8))
    }

    private var firstPassSpineStatus: some View {
        HStack(spacing: 8) {
            if let spine = vm.firstPassSpine {
                Image(systemName: "checkmark.circle.fill")
                    .foregroundStyle(.green)
                Text("First-pass spine wired")
                    .font(.subheadline.weight(.semibold))
                Text("\(spine.beats.count) beats")
                    .font(.caption)
                    .foregroundStyle(.secondary)
                Text("(\(spine.throughline.prefix(60))...)")
                    .font(.caption2)
                    .foregroundStyle(.secondary)
                    .lineLimit(1)
            } else {
                Image(systemName: "exclamationmark.triangle.fill")
                    .foregroundStyle(.orange)
                Text("No first-pass spine")
                    .font(.subheadline)
                    .foregroundStyle(.orange)
                Text("V6–V10 paths require a completed Phase 1 spine + Gap Detection.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
        }
        .padding(10)
        .frame(maxWidth: .infinity, alignment: .leading)
        .background(vm.firstPassSpine != nil ? Color.green.opacity(0.08) : Color.orange.opacity(0.08))
        .clipShape(RoundedRectangle(cornerRadius: 8))
    }
    ...
    private var dependencyStatusRow: some View {
        HStack(spacing: 8) {
            if vm.isLoadingDependencies {
                ...
                Text("Dependencies will be wired from Phase 1.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
        }
        ...
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                Text("Paths")
                    .font(.subheadline.weight(.semibold))
                Spacer()
                Button(vm.enabledPaths.count == ArcPath.allCases.count ? "Deselect All" : "Select All") {
                    if vm.enabledPaths.count == ArcPath.allCases.count {
                        vm.enabledPaths = []
                    } else {
                        vm.enabledPaths = Set(ArcPath.allCases)
                    }
                }
                .font(.caption)
            }

            // Original Paths (P1–P5)
            Text("Original Paths")
            ...
            HStack {
                ...
                Spacer()
                Button("V-Paths Only") {
                    vm.enabledPaths = Set(ArcPath.allCases.filter(\.isPass2Only))
                }
                .font(.caption2)
                Button("P-Paths Only") {
            ...
            ForEach(ArcPath.allCases.filter(\.isPass2Only)) { path in
                pathToggleRow(path)
            }
        }
    }

    private func pathToggleRow(_ path: ArcPath) -> some View {
        HStack(spacing: 8) {
            Toggle("", isOn: Binding(
            ...
            if let mirror = path.mirroredPPath {
                Text("(\(mirror.rawValue))")
                    .font(.caption2)
                    .foregroundStyle(.secondary)
            }
    ...
    // MARK: - Shared Preprocessing Section

    private func preprocessingCollapsibleSection(_ calls: [ArcPathCall]) -> some View {
        let totalTokens = calls.compactMap(\.telemetry).reduce(0) { $0 + $1.totalTokens }
        return CollapsibleSection(
            title: "Shared Preprocessing",
            icon: "doc.text.magnifyingglass",
            isExpanded: $isPreprocessingExpanded,
            count: "\(calls.count) calls | \(totalTokens) tok"
        ) {
            VStack(alignment: .leading, spacing: 8) {
                Text("Split content extraction: base inventory from original rambling + supplemental inventory from gap responses. Merged with [SUP] tags for all V-paths.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        HStack {
                            ...
                            if let t = call.telemetry {
                                FlowLayout(spacing: 6) {
                                    telemetryPill("in: \(t.promptTokens)", icon: "arrow.up.doc")
                                    telemetryPill("out: \(t.completionTokens)", icon: "arrow.down.doc")
                                }
                            }
    ...
    private func pathResultContent(_ result: ArcPathResult) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            // Gap coverage validation (V-paths only)
            if let gapCoverage = result.gapValidationResult {
                gapCoverageDisplay(gapCoverage)
            }
        }
    }

    private func gapCoverageDisplay(_ coverage: GapCoverageResult) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            Text("Gap Coverage")
                .font(.caption.weight(.semibold))

            FlowLayout(spacing: 6) {
                let coveredCount = coverage.coveredGapIds.count
                let uncoveredCount = coverage.uncoveredGapIds.count
                let total = coveredCount + uncoveredCount
                validationPill("Covered: \(coveredCount)/\(total)",
                              ok: uncoveredCount == 0)
                if uncoveredCount > 0 {
                    validationPill("Uncovered: \(uncoveredCount)",
                                  ok: false)
                }
            }

            if !coverage.coverageSummary.isEmpty {
    ...
    private var comparisonContent: some View {
        ...
        if completed.isEmpty {
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                comparisonRow("Gap Covered", values: completed.map {
                    guard let g = $0.gapValidationResult else { return "—" }
                    let total = g.coveredGapIds.count + g.uncoveredGapIds.count
                    return "\(g.coveredGapIds.count)/\(total)"
                })
